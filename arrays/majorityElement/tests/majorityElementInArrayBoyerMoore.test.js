const majorityElementInArrayBoyerMoore = require("../majorityElementInArrayBoyerMoore");

describe("majorityElementInArrayBoyerMoore", () => {
  test("returns true when the array has a majority element", () => {
    const arr = [1, 2, 3, 2, 2, 2, 5, 4, 2];
    expect(majorityElementInArrayBoyerMoore(arr)).toBe(true);
  });

  test("returns false when the array does not have a majority element", () => {
    const arr = [1, 2, 3, 4, 5, 6];
    expect(majorityElementInArrayBoyerMoore(arr)).toBe(false);
  });
